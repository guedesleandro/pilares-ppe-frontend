...
import { parseDatePt, formatDateToPt, applyDateMask, createMaskedInputHandler } from "@/lib/utils";
import {
  ApiError,
  listMedications,
  ...
  useEffect(() => {
    if (initialData && open) {
      form.reset({
        ...
        birth_date: formatDateToPt(new Date(initialData.birth_date)),
        process_number: initialData.process_number || "",
        treatment_location: initialData.treatment_location,
        preferred_medication_id: initialData.preferred_medication_id || "none",
      });
    }
  }, [initialData, open, form]);

  async function onSubmit(data: PatientEditFormData) {
    setIsLoading(true);
    ...
    try {
      ...
      if (error instanceof ApiError) {
        toast.error(
          error.message ??
            "Não foi possível atualizar o paciente. Verifique os dados e tente novamente.",
        );
      } else {
        console.error("Erro ao atualizar paciente:", error);
        toast.error("Erro inesperado ao atualizar o paciente.");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Atualize as informações de <strong>{patientName}</strong>.
          </DialogDescription>
        </DialogHeader>
