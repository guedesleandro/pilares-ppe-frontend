...
import type { WeightLossRankingItem } from "@/lib/api";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

type WeightLossRankingTableProps = {
  data: WeightLossRankingItem[];
  startDate?: string | null;
  endDate?: string | null;
};

export function WeightLossRankingTable({
  data,
  startDate,
  endDate,
}: WeightLossRankingTableProps) {
  if (!data || data.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Ranking de Perda de Peso</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex h-[200px] items-center justify-center text-muted-foreground">
            Nenhum paciente com sessões no período selecionado
          </div>
        </CardContent>
      </Card>
    );
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Perda de Peso</TableHead>
            ...
            {data.map((item) => (
              <TableRow key={item.patient_id}>
                ...
                <TableCell className="text-right font-semibold text-green-600">
                  {formatNumberPt(item.weight_loss_kg, 1, 1)} kg
                </TableCell>
